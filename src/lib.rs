		...
		fn on_initialize(now: T::BlockNumber) -> Weight {
			...
			if ! queued.is_empty() {
				...
				for (order, (index, mut s)) in queued.into_iter().enumerate() {
					...
					let hard_deadline = s.priority <= HARD_DEADLINE;
					let test_weight =
...
impl<T: Config> Anon<T::BlockNumber, <T as Config>::Call, T::PalletsOrigin>
	for Pallet<T>
{
...
impl<T: Config> Named<T::BlockNumber, <T as Config>::Call, T::PalletsOrigin>
	for Pallet<T>
{
	type Address = TaskAddress<T::BlockNumber>;
	type Hash = T::Hash;
